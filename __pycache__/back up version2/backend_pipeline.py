...
def run_raw_extraction(progress_cb=None):
    ...
    def run_scraper_session(session_id, keyword, csv_file):
        ...
        try:
            ...
            # Prefer virtual environment if present
    ...
    for i, (keyword, csv_file) in enumerate(zip(SEARCH_KEYWORDS, SESSION_CSV_FILES), 1):
        ...
        thread.start()
        time.sleep(10)  # Stagger launches to avoid resource conflicts
...
def cancel_extraction():
    """Cancel all running scraper sessions"""
    if not STATUS_FILE.exists():
        return False
    ...
    for i, pid in enumerate(pids, 1):
        try:
            ...
            log_backend(f"⚠️  Failed to cancel Session {i}: {e}")
