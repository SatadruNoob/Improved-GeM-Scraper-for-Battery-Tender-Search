...
def read_status_file():
    """Read status file with error handling and return parsed JSON"""
...
# Clear any cached status to ensure fresh read
status = read_status_file()
...
with col_btn1:
    if st.button(
        ...
        disabled=is_extraction_running(),
        type="primary"
    ):
...
with col_btn2:
    if st.button(
        "🛑 Cancel Extraction",
        disabled=not is_extraction_running(),
        type="secondary"
    ):
...
with col_btn3:
    if st.button(
        "📊 Run Analysis",
        disabled=is_extraction_running() or not CSV_FILE.exists(),
        type="secondary"
    ):
...
if st.session_state.data_ready and EXCEL_FILE.exists():
    ...
    with col_filter2:
        show_unchecked_only = st.checkbox(
            "Show Only Unchecked Bids",
            value=st.session_state.show_only_unchecked
        )
        st.session_state.show_only_unchecked = show_unchecked_only
...
# ────────────────────────────────────────
# CRITICAL FIX: Force refresh more aggressively when extraction is running
if is_extraction_running() or has_active_sessions:
    # Update timestamp
    st.session_state.last_update_time = time.time()
